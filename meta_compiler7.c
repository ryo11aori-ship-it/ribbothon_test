...
int threes = val / 3;
int ones = val % 3;
for(int i=0; i<threes; i++) emit(c3);
for(int i=0; i<ones; i++) emit(c1);
}
int main() {
int c;
while((c = getchar()) != EOF) {
...
int addr = get_sym(); move_to(addr); emit('7'); loop_stack[loop_sp++] = addr;
}
else if(c == 'E') {
if(loop_sp > 0) { int addr = loop_stack[--loop_sp]; move_to(addr); emit('8'); }
}
else if(c == 'Z') {
int addr = get_sym(); move_to(addr); emit('7'); emit('4'); emit('8');
}
else if(c == 'A') {
...
int addr = get_sym(); move_to(addr); emit('5');
}
else if(c == '$') {
ungetc('$', stdin);
